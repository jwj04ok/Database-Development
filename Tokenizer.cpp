  ...
  StatusResult Tokenizer::tokenize() {
    ...
    while(!input.eof()) {
      ...
      if(isPunctuation(theChar)) {
          TokenType atype=TokenType::punctuation;
          switch(theChar){
              case '(':atype=TokenType::lparen;
                  break;
              case ')':atype = TokenType::rparen;
                  break;
              case ',':atype = TokenType::comma;
                  break;
              case ';':atype = TokenType::semicolon;
                  break;
              default:atype = TokenType::punctuation;
                  break;
          }
        Token theToken{atype, Keywords::unknown_kw, Operators::unknown_op};
        ...
        theToken.data=readWhile(isNumber);
          if (theToken.data.find(".", 0) != std::string::npos){
              theToken.keyword = Keywords::float_kw;
          }else{theToken.keyword = Keywords::integer_kw;}
        tokens.push_back(theToken);
      }
